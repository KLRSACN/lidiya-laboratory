...
def load_installation_metadata(root: Path):
    root=root.resolve(strict=False); path=root/".lidiya"/"installation.json"
    if not path.is_file(): raise CanaryError("installation metadata required for Windows owner canary")
    try: data=json.loads(path.read_text(encoding="utf-8-sig"))
    except Exception as exc: raise CanaryError("invalid installation metadata json") from exc
    if data.get("schema_version") != "1.0": raise CanaryError("unsupported installation schema")
    iid=str(data.get("installation_id","")).strip()
    try: uuid.UUID(iid)
    except Exception as exc: raise CanaryError("installation_id must be UUID") from exc
    bound_root=Path(str(data.get("install_root","") or "")).resolve(strict=False)
    if bound_root != root: raise CanaryError("installation root mismatch")
    if data.get("privilege") != "USER_SPACE": raise CanaryError("installation privilege mismatch")
    if data.get("transport") != "LOOPBACK_AND_WORKSPACE_SPOOL": raise CanaryError("installation transport mismatch")
    return data
...
def run_windows_owner_canary(workspace_root):
    ...
    install_fp=_hash(meta)
    out={"mode":"WINDOWS_FIXED_HARMLESS_ECHO","command_id":result["command_id"],"stdout":result["stdout"],"stderr":result["stderr"],"exit_code":result["exit_code"],"evidence_sha256":result["evidence_sha256"],"authorization_ref":AUTH_REF,"arbitrary_command_input":False,"installation_id":meta["installation_id"],"installation_fingerprint":install_fp,"install_root":str(root),"provenance":{"source":"LOCAL_OWNER_WINDOWS_EXECUTION","observed_by":"LOCAL_CANARY"},"promotion_status":"REAL_LOCAL_CANARY_EVIDENCE_CANDIDATE_UNATTESTED"}; out["canary_sha256"]=_hash(out); (root/".lidiya"/"local_canary_evidence.json").write_text(json.dumps(out,sort_keys=True,indent=2,ensure_ascii=False),encoding="utf-8"); return out
