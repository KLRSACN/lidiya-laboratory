...
def _filter_positions(pid: str) -> list[int]:
    raw = pid.encode("utf-8")
    out = []
    for i in range(PULSE_FILTER_HASHES):
        d = hashlib.sha256(bytes([i]) + raw).digest()
        out.append(int.from_bytes(d[:8], "big") % PULSE_FILTER_BITS)
    return out

def _filter_int(hex_value: str) -> int:
    if len(hex_value) != PULSE_FILTER_HEX_LEN:
        raise HeartbeatError("invalid pulse filter length")
    try:
        return int(hex_value, 16)
    except ValueError as exc:
        raise HeartbeatError("invalid pulse filter encoding") from exc

def _filter_contains(hex_value: str, pid: str) -> bool:
    bits = _filter_int(hex_value)
    return all(bits & (1 << p) for p in _filter_positions(pid))

def _filter_add(hex_value: str, pid: str) -> str:
    bits = _filter_int(hex_value)
    for p in _filter_positions(pid):
        bits |= 1 << p
...
class HeartbeatEngine:
    ...
    Canonical automatic pulses (pulse_id=None) are identified by the persisted
    monotonic sequence and scheduled due-time. They never consult the
    probabilistic replay filter, so a Bloom false positive cannot suppress
    canonical liveness. The bounded persisted filter is only a conservative
    replay guard for caller-supplied pulse IDs; inserted IDs have no false
    negatives and therefore remain non-executing across restart.
    """
    ...
    def _load_or_initialize(self) -> HeartbeatState:
        ...
        _filter_int(s.pulse_filter_hex)
    ...
    def _cas_guard(self, g: Optional[int]) -> None:
        if g is not None and g != self.state.writer_generation:
            raise StaleWriterError(
                f"stale writer generation: expected={g} actual={self.state.writer_generation}"
            )
        ...
        *,
        ...
        if not self.due(now):
            return PulseResult(
                ...
                pulse_id or "",
                False,
                self.state.pulse_sequence,
                self.state.event_cursor,
                self.state.endpoint_status,
                False,
                self.state.writer_generation,
                dict(ZERO_EXPERIENCE_DELTA),
            )

        scheduled_at = (
            self.state.next_due_at if self.state.next_due_at is not None else now
        )
        proposed = self.state.pulse_sequence + 1
        automatic_canonical = pulse_id is None
        pid = (
            canonical_pulse_id(proposed, scheduled_at)
            if automatic_canonical
            else str(pulse_id)
        )

        # Caller-supplied IDs are conservatively checked against the bounded
        # persistent replay filter. Canonical automatic liveness never depends
        # on probabilistic membership and therefore cannot deadlock on a false
        # positive.
        if not automatic_canonical and _filter_contains(self.state.pulse_filter_hex, pid):
            return PulseResult(
                "DUPLICATE_NO_OP",
                pid,
                False,
        ...
        self.state.pulse_filter_hex = _filter_add(self.state.pulse_filter_hex, pid)
        ...
        compact = (
            self.state.pulse_sequence % COMPACT_CADENCE_PULSES == 0 or anomaly
        )
        if compact:
