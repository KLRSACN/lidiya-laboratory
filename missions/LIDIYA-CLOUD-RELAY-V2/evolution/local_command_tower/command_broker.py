...
UNSAFE_SYNTAX=re.compile(r"(\&\&|\|\||[|`;]|\$\(|`|\$\{|\$env:|%[A-Za-z_][A-Za-z0-9_]*%|\\\\)")
WINDOWS_ABS=re.compile(r"(?i)(?:^|[\s\"'])[A-Z]:[\\/]")
...
class CommandPolicy:
    ...
    def validate(self, env: Dict[str,Any]) -> Dict[str,Any]:
        ...
        if WINDOWS_ABS.search(cmd) or UNIX_ABS.search(cmd) or re.search(r"(^|\s)\.\.(?:[\\/]|$)",cmd): raise BrokerRejected("path escape syntax")
        try: head=shlex.split(cmd,posix=False)[0].strip("\"'").lower()
        except Exception as e: raise BrokerRejected("cannot tokenize command") from e
        allowed=SAFE_CMD_HEADS if shell=="cmd" else SAFE_PS_HEADS
...
class CommandBroker:
    def __init__(self, workspace_root: str|Path, *, execute_enabled: bool=False, executor: Optional[Callable[[Dict[str,Any]],Dict[str,Any]]]=None):
        self.policy=CommandPolicy(Path(workspace_root)); self.execute_enabled=execute_enabled; self.executor=executor or SubprocessExecutor(); self._seen={}
    def execute(self, envelope: Dict[str,Any]) -> Dict[str,Any]:
        ...
        if not self.execute_enabled: result={"stdout":"","stderr":"","exit_code":None,"disposition":"DRY_RUN_VALIDATED_NOT_EXECUTED"}
            ...
            try:
                result=self.executor(env); result["disposition"]="EXECUTED"
            except subprocess.TimeoutExpired:
                result={"stdout":"","stderr":"TIMEOUT","exit_code":124,"disposition":"TIMEOUT"}
