...
from gearbox_secretary_runtime_freshness_shadow_v02 import (
    RuntimeClockCheckpoint,
    verify_clock_checkpoint,
)
from gearbox_secretary_signal_shadow_v01 import NEUTRAL_PRESSURE, neutral_secretary_projection
...
def unresolved_chain_break(reason: str) -> ClockRecoveryProjection:
    """Fail closed without projecting a permanent terminal N/R identity state."""
...
def establish_replacement_clock_epoch(
    *,
    ...
    """Bind an authenticated replacement checkpoint/key epoch to an external monotonic floor.

    The external receipt authenticates the epoch metadata and verified checkpoint fingerprint.
    This is non-formal shadow semantics: provider identity strings do not prove physical
    independence, and the clock secret remains synthetic/test-only unless production evidence exists.
    """
...
def complete_authenticated_reentry(
    *,
    ...
    """Exit CLOCK_RECOVERY_REQUIRED only under fresh signed Home/MISSION authority.

    A recovered clock epoch never resurrects pre-break secretary pressure, anti-thrash history,
    terminal hold, Experience, appraisal, personality, trauma or relief semantics.
    """
    ...
    return ClockRecoveryProjection(
        ...
        reason=f"fresh authority {envelope.decision_id} accepted after authenticated clock epoch recovery",
    )


def pressure_history_dataflow_exclusion() -> dict[str, Any]:
    """Machine-readable guardrail for SPIRIT-MOD-GB21-044 dataflow review."""
    return {
