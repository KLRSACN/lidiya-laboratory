...
class AdapterTests(unittest.TestCase):
    def source(self, **changes):
        values = dict(source_type="drive_doc", source_ref="doc1", fingerprint="abc", confidence=0.9, verified_count=2, last_verified="2026-08-13", ttl=3600, level="L1")
    ...
    def test_metadata_changes_hash(self):
        self.assertNotEqual(build_manifest([self.source()])["manifest_sha256"], build_manifest([self.source(ttl=7200)])["manifest_sha256"])
    ...
    def test_full_load_is_rejected(self):
        with self.assertRaises(ValueError):
            route_sources(build_manifest([self.source()]), ["ALL"])

    def test_route_is_bounded(self):
        items = [self.source(source_ref="doc"+str(i), fingerprint=str(i)) for i in range(20)]
        self.assertEqual(len(route_sources(build_manifest(items), ["L1"], max_sources=8)), 8)

    def test_bootstrap_order(self):
        self.assertEqual(bootstrap_l0("index", ["00", "31", "32", "33"])["refs"], ["00", "31", "32", "33"])
