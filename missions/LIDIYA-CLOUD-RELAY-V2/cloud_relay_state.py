...
UTC = timezone.utc
MAX_CONSUMED_PACKET_HISTORY = 64
...
ROLE_FOR_STATUS: dict[str, str] = {
    "IDLE": "LCR-A", "COORDINATING": "LCR-A", "READY_FOR_BUILDER": "LCR-B",
    "BUILDING": "LCR-B", "READY_FOR_VERIFY": "LCR-C", "VERIFYING": "LCR-C",
    "STEP_DONE": "LCR-A", "PROJECT_DONE": "LCR-A", "METABOLIZE": "LCR-A",
    "HUMAN_GATE": "HUMAN",
}
...
def claim(state: dict[str, Any], role: str, owner: str, ttl_seconds: int = 900, at: datetime | None = None) -> dict[str, Any]:
    at = at or now_utc()
    ...
    updated = dict(state)
    updated["lease"] = {"role": role, "owner": owner, "claimed_at": at.isoformat(), "expires_at": (at + timedelta(seconds=ttl_seconds)).isoformat()}
    return updated
...
def packet(*, state: dict[str, Any], run_id: str, source_role: str, target_role: str, status: str, task: str, acceptance: list[str], evidence: list[dict[str, Any]] | None = None, result: str | None = None, lesson: str | None = None, disposition: str = "candidate") -> dict[str, Any]:
    payload: dict[str, Any] = {
        "schema_version": state["schema_version"], "mission_id": state["mission_id"], "run_id": run_id,
        "step_id": state["step_id"], "attempt": state["attempt"], "source_role": source_role,
        "target_role": target_role, "status": status, "parent_packet_sha256": state.get("last_packet_sha256"),
        "created_at": now_utc().isoformat(), "lease_owner": (state.get("lease") or {}).get("owner"),
        "lease_expires_at": (state.get("lease") or {}).get("expires_at"), "task": task,
        "acceptance": acceptance, "candidate_ref": state.get("candidate_ref"), "evidence": evidence or [],
        "result": result, "lesson": lesson, "disposition": disposition,
    }
    payload["packet_sha256"] = sha256_json(payload)
    return payload


def _consumed_packet_history(state: dict[str, Any]) -> list[str]:
    history = [value for value in state.get("consumed_packet_sha256", []) if isinstance(value, str) and value]
    legacy_last = state.get("last_packet_sha256")
    if legacy_last and legacy_last not in history:
        history.append(legacy_last)
    return history[-MAX_CONSUMED_PACKET_HISTORY:]
...
def consume_once(state: dict[str, Any], packet_value: dict[str, Any]) -> dict[str, Any]:
    ...
    history = _consumed_packet_history(state)
    if packet_sha in history:
        raise RelayStateError("duplicate packet consumption")
    updated = dict(state)
    history.append(packet_sha)
    updated["consumed_packet_sha256"] = history[-MAX_CONSUMED_PACKET_HISTORY:]
